 ...
 -->
		...
		<input type="hidden" name="page" value="${ currentPage }">
			<table align="center" width="700" border="1" cellspacing="0"
				...
				<tr>
					<th width="120">제 목</th>
					<td><input type="text" name="boardTitle" size="70" value="${ board.boardTitle }"></td>
				</tr>
				<tr>
					<th>작성자</th>
					<td><input type="text" name="boardWriter" readonly
						value="${ board.boardWriter }"></td>
				</tr>
				<tr>
					<th>첨부파일</th>
					<td><c:if test="${ !empty board.boardOriginalFileName }">
						...
						</c:if> <%-- name 속성의 이름은 property 명과 다르게 지정함
					파일업로드 실패시 파일명만 문자열로 command 객체에 저장되지 않게 하기 위함
				 --%></td>
				</tr>
				<tr>
					<th>내 용</th>
					<td><textarea name="boardContent" rows="5" cols="70">${ board.boardContent }</textarea></td>
				</tr>
				<tr>
					<th colspan="2"><input type="submit" value="수정하기">&nbsp; 
					<input type="reset" value="수정취소"> &nbsp; 
					<input type="button" value="이전페이지로 이동" onclick="history.go(-1); return false;">&nbsp;
					<input type="button" value="목록" onclick="location.href='blist.do?page=${ currentPage }'; return false;">&nbsp;</th>
				</tr>
			</table>
		</form>
	</c:if>
	...
	<c:if test="${ board.boardLev gt 1 }"><%--gt (greater than) : > 연산자임 : 1보다 크냐 --%>
	<h1 align="center">${ board.boardNum } 번 게시글 댓글 | 대댓글 수정 페이지 </h1>
	<br>
		<form action="breplyupdate.do" method="post" enctype="multipart/form-data">
		<input type="hidden" name="boardNum" value="${ board,.boardNum }">
		<input type="hidden" name="page" value="${ currentPage }">
